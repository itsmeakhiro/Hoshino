const LevelingSystem = require("../../resources/plugins/level/utils");
...
const command = {
   ...
   */
  async deploy(ctx) {
    const { chat, event, styler, fonts, hoshinoDB, BalanceHandler } = ctx;
    ...
    const subcommands = [
      {
        ...
        deploy: async function (/** @type {HoshinoLia.CommandContext} */ ctx) {
          ...
          await balanceHandler.setBalance(100);
      ...
      },
      {
        ...
        deploy: async function (/** @type {HoshinoLia.CommandContext} */ ctx) {
          ...
          if (!userData) {
            return chat.send(
              "You are not registered! Use !profile register first."
            );
          }
