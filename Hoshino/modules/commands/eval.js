...
const command = {
  ...
  async deploy({ chat, args, fonts, event }) {
    ...
    if (args.length === 0 && (!attachments || attachments.length === 0)) {
      return chat.send(
        fonts.sans(
          "Please provide code to evaluate or a file (supports .js, .cjs, .ts, .py, .cpp, .c, .java, .rb, .go)"
        )
      );
    }
    ...
    const executeCode = async () => {
      ...
      if (fileExt === ".c") {
        ...
        return new Promise((resolve, reject) => {
          exec(
            `gcc ${tempFile} -o ${outFile} && ${outFile}`,
            (error, stdout, stderr) => {
              fs.remove(tempFile);
              fs.remove(outFile);
              if (error) return reject(stderr || error.message);
      ...
      if (fileExt === ".java") {
        ...
        return new Promise((resolve, reject) => {
          exec(
            `javac ${tempFile} && java -cp ${tempDir} ${className}`,
            (error, stdout, stderr) => {
              fs.remove(tempFile);
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          if (typeof installError !== "string") return;
          return chat.send(
            fonts.bold("Install Error:\n") + fonts.bold(installError)
          );
        }
      }
